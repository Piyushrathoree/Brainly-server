...
if (process.env.GOOGLE_CLIENT_ID &&
    ...
    passport_1.default.use(new passport_google_oauth20_1.Strategy({
        ...
        try {
            let user = await oAuth_model_1.AuthUser.findOne({
    ...
    console.warn("[oauth] Google OAuth not configured; missing GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET/CALLBACK_URL");
}
if (process.env.GITHUB_CLIENT_ID &&
    ...
    console.warn("[oauth] GitHub OAuth not configured; missing GITHUB_CLIENT_ID/GITHUB_CLIENT_SECRET/CALLBACK_URL");
}
const googleCallback = (req, res) => {
    const user = req.user;
    const secret = process.env.JWT_SECRET || "dev-jwt-secret";
    const token = (0, jwt_1.signJwt)({ id: String(user._id), email: user.email }, secret, 7 * 24 * 60 * 60);
